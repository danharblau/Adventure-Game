...
def open_safe(items):
    ...
    if "password" in items:
        print_pause("You remember you had already opened it and found the keys")
        print_pause("There doesn't seem to be much to do here.")
    else:
        if "mural" in items:
            print_pause("You remember the mural and notice that the password "
                        "for the safe is only three digits long, so you try 314")
...
def open_door(items):
    ...
    if "mural" in items:
        print_pause("You remember the mural, but you need more to open the door.")
        if "password" in items:
            print_pause("Fortunately, you had found the keychain!")
            print_pause("Congratulatons! After a few tries to manage to open the door!"
                        "You have successfully freed yourself!!!")
        else:
            print_pause("Come on, you're better than this, you need more clues!"
